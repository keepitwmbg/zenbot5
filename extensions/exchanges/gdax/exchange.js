    ...
    getQuote: async opts => {
      ...
      if (websocket_cache[opts.product_id]) {
        ...
        if (ticker.best_ask && ticker.best_bid) {
          return {
            err: null,
            data: { bid: ticker.best_bid, ask: ticker.best_ask },
          };
        }
      }
      var func_args = [].slice.call(arguments);
      ...
      try {
        let ticker = await client.getProductTicker(opts.product_id);
        if (ticker.bid || ticker.ask) {
          ...
          return {
            err: { code: 'ENOTFOUND', body: opts.product_id + ' has no liquidity to quote' },
            data: {},
          };
        }
      } catch (err) {
