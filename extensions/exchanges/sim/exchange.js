...
module.exports = function sim(conf, s) {
  ...
  function processBuy(buy_order, trade) {
    ...
    // Update balance
    balance.asset = n(balance.asset).add(size).subtract(fee).format({ mantissa: 8 });
    balance.currency = n(balance.currency).subtract(total).format({ mantissa: 8 });
  ...
  function processSell(sell_order, trade) {
    ...
    balance.currency = n(balance.currency).add(total).subtract(fee).format({ mantissa: 8 });
