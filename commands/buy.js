...
export default (program, conf) => {
  ...
  program
    ...
    .option(
      ...
      Number,
      conf.markdown_buy_pct
    )
    .option(
      "--order_adjust_time <ms>",
      "adjust bid on this interval to keep order competitive",
      Number,
      conf.order_adjust_time
    )
    .option(
      "--order_poll_time <ms>",
      "poll order status on this interval",
      Number,
      conf.order_poll_time
    )
    .option(
      "--max_slippage_pct <pct>",
      "avoid buying at a slippage pct above this float",
      conf.max_slippage_pct
    )
        ...
        if (s.api_order && !_.isEmpty(s.api_order)) {
          ...
          console.log(
            "order status: ".grey +
              ...
              ", ".grey +
              n(quote.bid).subtract(s.api_order.price).format("0.00000000")
                .red +
              " below best bid, ".grey +
              n(s.api_order.filled_size).divide(s.api_order.size).format("0.0%")
                .green +
              " filled".grey
          );
