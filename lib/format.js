...
let max_fc_width = 0;
let formatAsset = (amt, asset) => {
  return n(amt).format({ mantissa: 8 }) + ' ' + asset;
};
let formatPercent = ratio => {
  return (ratio >= 0 ? '+' : '') + n(ratio).format({ output: 'percent', mantissa: 2 });
};
let formatCurrency = (amt, currency, omit_currency, color_trick, do_pad) => {
  let str;
  let fstr;
  amt > 999
    ? (fstr = '0.00')
...
export { formatAsset, formatPercent, formatCurrency, getMantissa };
