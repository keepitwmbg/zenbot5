...
export default async function (s, conf) {
  ...
  if (so.strategy) {
    s.strategy = await import(path.resolve(__dirname, `../extensions/strategies/${so.strategy}/strategy.js`));
  ...
  let syncBalance = async () => {
    ...
    let result;
    result = await s.exchange.getBalance({ currency: s.currency, asset: s.asset });
    // balance
    // {
    //   asset: 0.0003,
    //   currency: 77856.3852,
  ...
  let placeOrder = async (type, opts) => {
    if (!s[type + '_order']) {
      s[type + '_order'] = {
        price: opts.price,
        size: opts.size,
  ...
  let doOrder = async (size, price, expected_fee, is_taker, signal) => {
    let opts = {
      size: size,
      price: price,
    ...
    return await placeOrder(signal, opts);
    // {
    //   err: null,
    //   data: null,
    // }
  ...
  let executeOrder = (order, trade_type) => {
    ...
    if (so.stats) {
      ...
      let slippage = n(my_trade.slippage).format({ output: 'percent', mantissa: 4 });
  ...
  //   status: "open",
  // }
  let cancelOrder = async (order, type, do_reorder) => {
    let opts = { order_id: order.order_id, product_id: s.product_id };
    await s.exchange.cancelOrder(opts);
    let result = await checkHold(do_reorder, order, type);
    let resultDA = await doOrderAgain(result.err, result.data, order, type);
    return resultDA;
  };

  let doOrderAgain = async (error, hasOrder, order, signal) => {
    let size = order.size;
    if (error) {
  ...
  let checkHold = async (do_reorder, order, type) => {
    let opts = { order_id: order.order_id, product_id: s.product_id };
    ...
    if (on_hold && s.balance.currency_hold > 0) {
      ...
      return { err: null, data: do_reorder ? null : false };
    }
  };
  ...
  let checkOrder = async (order, type) => {
    ...
    let result = await s.exchange.getOrder({ order_id: order.order_id, product_id: s.product_id });
    // result = {
    //   err: null,
    //   data: {
    //     id: 28919868791,
  ...
  let tradeProcessingQueue = async.queue(function ({ trade, is_preroll }, callback) {
