...
export default async (s, conf) => {
  const __dirname = dirname(fileURLToPath(import.meta.url));

  let so = s.options;
  if (_.isUndefined(s.exchange)) {
    if (so.mode !== 'live') {
      let ex = await import(path.resolve(__dirname, '../extensions/exchanges/sim/exchange.js'));
      s.exchange = ex.default(conf, s);
    } else {
      let ex = await import(path.resolve(__dirname, `../extensions/exchanges/${so.selector.exchange_id}/exchange.js`));
  ...
  if (so.strategy) {
    s.strategy = (await import(path.resolve(__dirname, `../extensions/strategies/${so.strategy}/strategy.js`))).default;
    if (s.strategy.getOptions) {
      s.strategy.getOptions.call(s.ctx, s);
    }
    if (s.strategy.orderExecuted) {
      eventBus.on('orderExecuted', async type => {
        await s.strategy.orderExecuted(s, type, executeSignal);
      });
    }
  }
  ...
  let nextSellForQuote = (s, quote) => {
    if (s.next_sell_price) {
      let result = n(s.next_sell_price).format(getMantissa(s.product.increment));
      return result;
    } else {
  ...
  let syncBalance = async () => {
    ...
    let balance = await s.exchange.getBalance({
      currency: s.currency,
      asset: s.asset,
    });
    // balance
    ...
    s.asset_capital = n(s.balance.asset).multiply(quote.ask).value(); // the value of the virtual currency held
    ...
    if (!s.start_capital) {
      ...
      s.real_capital = n(s.balance.currency).add(s.asset_capital).value(); // total asset value
    ...
    return {
      net_currency: s.net_currency, // cash assets
      currency: s.currency, // cash unit
      net_asset: s.balance.asset, // coin assets
      asset: s.asset, // coin unit
      real_capital: s.real_capital, // total assets = cash assets + coin assets
      quote: quote,
    };
  };
  ...
  let isOrderTooSmall = (product, quantity, price) => {
    if (product.min_size && Number(quantity) < Number(product.min_size)) return true;
    if (product.min_total && n(quantity).multiply(price).value() < Number(product.min_total)) return true;
    return false;
  };
  ...
  let executeSignal = async (signal, size, is_reorder, is_taker, reverseCalled) => {
    ...
    if (signal === 'buy') {
      ...
      tradeable_balance = n(s.balance.deposit)
        .divide(100 + fee)
        .multiply(buy_pct);
      expected_fee = n(trade_balance).subtract(tradeable_balance).add(0.00000001).format({ mantissa: 8 }); // round up as the exchange will too
      if (buy_pct + fee < 100) {
        size = n(tradeable_balance).divide(price).format(getMantissa(s.product.asset_increment, 8));
      } else {
        size = n(trade_balance).subtract(expected_fee).divide(price).format(getMantissa(s.product.asset_increment, 8));
      }
      ...
      // 2023-05-31 11:38:25 - preparing buy order over 0.00017404 BTC of 673.490 JPY (10%) tradeable balance with a expected fee of -0.13469797 JPY (-0.02%)
      debug.msg(
      ...
      if (
        ...
        return setTimeout(async () => {
          if (s.last_signal === signal) {
            let result = await executeSignal(signal, size, true);
      ...
      if (conf.notifiers && !conf.notifiers.only_completed_trades) {
        pushMessage(
          ...
          'placing buy order at ' + formatCurrency(price, s.currency) + ', ' + formatCurrency(quote.bid - Number(price), s.currency) + ' under best bid\n'
        );
      }
      ...
      if (conf.notifiers && !conf.notifiers.only_completed_trades) {
        pushMessage(
          ...
          'placing sell order at ' + formatCurrency(price, s.currency) + ', ' + formatCurrency(Number(price) - quote.bid, s.currency) + ' over best ask\n'
        );
      }
    }
    ...
    // console.debug('resultPO:', resultPO);
    cb(resultPO.err, resultPO.data);
    return resultPO;
  };
  ...
  let checkOrder = async (order, type) => {
    ...
    if (api_order.status === 'done') {
      ...
      return { err: err, data: await syncBalance() };
    }
  ...
  let checkHold = async (order, type, doReorder) => {
    ...
    if (api_order) {
      ...
      if (api_order.filled_size) {
        order.remaining_size = n(order.size).subtract(api_order.filled_size).format(getMantissa(s.product.asset_increment, 8));
      }
    }
    await syncBalance();
    let on_hold;
  ...
  let executeOrder = (order, trade_type) => {
    ...
    if (order_type === 'maker' && s.exchange.makerFee) percentage_fee = s.exchange.makerFee;
    else if (order_type === 'taker' && s.exchange.takerFee) percentage_fee = s.exchange.takerFee;
    if (trade_type === 'sell')
      fee = n(order.size)
    ...
    let my_trade = {
      ...
      slippage: trade_type === 'sell' ? n(order.orig_price).subtract(price).divide(price).value() : n(price).subtract(order.orig_price).divide(order.orig_price).value(),
      type: trade_type,
  ...
  let writeReport = (is_progress, blink_off) => {
    ...
    process.stdout.write(
      moment(is_progress ? s.period.latest_trade_time : tb(s.period.time).resize(so.period_length).add(1).toMilliseconds()).format('YYYY-MM-DD HH:mm:ss')[is_progress && !blink_off ? 'bgBlue' : 'grey']
    );
    ...
    let volume_display = s.period.volume > 99999 ? abbreviate(s.period.volume, 2) : n(s.period.volume).format({ mantissa: 0 });
    ...
    if (orig_capital) {
      let asset_col = n(s.balance.asset).format(s.asset === 'BTC' ? { mantissa: 4 } : { mantissa: 8 }) + ' ' + s.asset;
      asset_col_width = Math.max(asset_col.length + 1, asset_col_width);
      process.stdout.write(z(asset_col_width, asset_col, ' ').white);
      let deposit_col = n(s.balance.deposit).format(isFiat() ? { mantissa: 2 } : { mantissa: 8 }) + ' ' + s.currency;
      ...
      if (so.deposit) {
        let currency_col = n(s.balance.currency).format(isFiat() ? { mantissa: 2 } : { mantissa: 8 }) + ' ' + s.currency;
        ...
        process.stdout.write(z(8, n(circulating).format({ output: 'percent', mantissa: 2 }), ' ').grey);
      }
  ...
  let withOnPeriod = async (trade, period_id) => {
    ...
    if (!s.in_preroll) {
      ...
      if (!so.manual) {
        ...
        // it won't happen here, no onPeriod method excute, but it is OK to retain it
        if (s.signal) {
          await executeSignal(s.signal);
          s.signal = null;
        }
      }
    }
  ...
  let tradeProcessingQueue = async.queue(async ({ trade, is_preroll }) => {
    await onTrade(trade, is_preroll);
  });
  ...
  let onTrades = async (trades, is_preroll) => {
    trades.sort((a, b) => {
  ...
  let onTrade = async (trade, is_preroll) => {
    let period_id;
    if (s.period && trade.time < s.period.time) {
      return;
    }
    let day = new Date(trade.time).getDate();
    if (s.last_day && day !== s.last_day) {
      s.day_count++;
    }
    s.last_day = day;
    if (!s.period) {
      // if new period trade exists, init onPeriod
      initBuffer(trade);
    }
    ...
    // if new period trade exists, strategy onPeriod.
    if (trade.time > s.period.close_time || (!s.in_preroll && so.mode != 'sim' && moment.duration(moment(now()).diff(s.period.last_try_trade)).asMinutes() >= so.interval_trade)) {
      period_id = tb(trade.time).resize(so.period_length).toString();
      s.period.last_try_trade = now();
      // excute onPeriod to make decision sell or buy
      s.strategy.onPeriod.call(s.ctx, s);
      writeReport();
      ...
      await withOnPeriod(trade, period_id);
    } else {
      await withOnPeriod(trade, period_id);
    }
  };
