...
// bucket
c.bucket = {};
c.bucket.bucketNameforSim = 'test1-369109_zenbot';
c.bucket.bucketPathforSim = 'sim';
...
// to enable therock trading, enter your API credentials:
c.bitbank = {};
c.bitbank.key = process.env.ZENBOT_BITBANK_API_KEY || 'YOUR-API-KEY';
c.bitbank.secret = process.env.ZENBOT_BITBANK_SECRET || 'YOUR-API-SECRET';

// Optional stop-order triggers:

// sell if price drops below this % of bought price (0 to disable) 止损百分比
c.sell_stop_pct = process.env.ZENBOT_SELL_STOP_PCT || 0;
// buy if price surges above this % of sold price (0 to disable) 回买百分比
c.buy_stop_pct = process.env.ZENBOT_BUY_STOP_PCT || 0;
// enable trailing sell stop when reaching this % profit (0 to disable)  止赢百分比
c.profit_stop_enable_pct = process.env.ZENBOT_PROFIT_STOP_ENABLE_PCT || 0;
// maintain a trailing stop this % below the high-water mark of profit  最高点止赢百分比
...
// buy with this % of currency balance (WARNING : sim won't work properly if you set this value to 100)  // 用这个百分比的货币余额购买
c.buy_pct = process.env.ZENBOT_BUY_PCT || 99;
// sell with this % of asset balance (WARNING : sim won't work properly if you set this value to 100)  // 以资产余额的这个百分比出售
c.sell_pct = process.env.ZENBOT_SELL_PCT || 99;
// ms to adjust non-filled order after  // ms 之后调整未成交订单
...
// ms to poll order status  // 轮询订单状态 ms
c.order_poll_time = process.env.ZENBOT_ORDER_POLL_TIME || 5000;
// ms to wait for settlement (after an order cancel)  // 等待ms结算（订单取消后）
c.wait_for_settlement = process.env.ZENBOT_WAIT_FOR_SETTLEMENT || 5000;
// % to mark down buy price for orders  // 为订单加价%
c.markdown_buy_pct = process.env.ZENBOT_MARKDOWN_BUY_PCT || 0;
// % to mark up sell price for orders  // 为订单加价%
c.markup_sell_pct = process.env.ZENBOT_MARKUP_SELL_PCT || 0;
// become a market taker (high fees) or a market maker (low fees)  // 成为做市商（高费用）或做市商（低费用）
c.order_type = process.env.ZENBOT_ORDER_TYPE || 'maker';
// when supported by the exchange, use post only type orders. //当交易所支持时，使用仅发布类型的订单。
c.post_only = process.env.ZENBOT_POST_ONLY || true;
// use separated fee currency such as binance'c BNB. //费用
...
// ms to poll new trades at  // ms 轮询新交易
c.poll_trades = process.env.ZENBOT_POLL_TRADES || 30000;
// amount of currency to start simulations with   // 开始模拟的货币数量
c.currency_capital = process.env.ZENBOT_CURRENCY_CAPITAL || 1000;
// amount of asset to start simulations with  // 开始模拟的资产数量
...
// number of periods to calculate RSI at   // 计算 RSI 的周期数
c.rsi_periods = process.env.ZENBOT_RSI_PERIODS || 14;
// period to record balances for stats   // 记录统计余额的时间段
...
c.notifiers.only_completed_trades = process.env.ZENBOT_NOTIFY_ONLY_COMPLETED_TRADES || false; // Filter to notifier'c messages for getting Commpleted Trades info.
...
c.notifiers.pushover.user = process.env.ZENBOT_PUSHOVER_USER_KEY; // this is your own user'c key (not application related)
